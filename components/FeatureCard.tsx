  ...
  return (
    <MotionArticle
      ...
      className={cn(
        ...
        dark
          ? "border-white/14 bg-white/[0.04] text-[#F8F6F1] hover:border-brand-gold"
      ...
      <div>
        <div
          className={cn(
            ...
            dark ? "border-brand-gold text-[#F8F6F1]" : "border-brand-navy bg-brand-gold text-brand-navy",
          )}
        >
          <Icon aria-hidden size={23} />
        </div>
        <h3 className="text-xl font-bold leading-tight sm:text-2xl">{title}</h3>
        <p className={cn("mt-4 text-lg leading-8", dark ? "text-[#F8F6F1]" : "text-ink-muted")}>{copy}</p>
      </div>
      <span className={cn("mt-8 inline-flex items-center gap-2 self-end text-base font-bold", dark ? "text-[#F8F6F1]" : "text-brand-navy")}>
